...
def brute_force_pids():
    ...
    def has_three_unique_digits(friend_code):
        """Check if a friend code has exactly three unique digits."""
        digits = set(friend_code.replace("-", ""))  # Remove hyphens and extract unique digits
        return len(digits) == 3
    ...
    for _ in range(total_pids):
        ...
        if has_three_unique_digits(friend_code):  # Check for exactly three unique digits
    ...
    messagebox.showinfo("Brute Force", f"Brute force complete! Found {found_codes} Friend Codes with exactly 3 unique digits.")
